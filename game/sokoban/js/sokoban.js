...
level1.split('\n').filter(rowData => !!rowData).forEach((rowData, row) => delete_space(rowData, row));
//level2.split('\n').filter(rowData => !!rowData).forEach((rowData, row) => delete_space(rowData, row));
//level3.split('\n').filter(rowData => !!rowData).forEach((rowData, row) => delete_space(rowData, row));

function delete_space(rowData, row) {
    cells[row] = [];
...
button_levels = document.querySelector('.levels_for_game');
div_all_levels = document.querySelector('.all_levels');
div_all_levels.classList.add('button_hide');
button_levels.addEventListener('click', function(){
    if(div_all_levels.classList.contains('button_hide')) {
        //console.log("aa")
        div_all_levels.classList.remove('button_hide');
    } else {
        div_all_levels.classList.add('button_hide');
    }
});

level = document.querySelectorAll('.spec_style');
console.log(level)
for(let i = 0; i < level.length; i++) {
    level[i].addEventListener('click', {handleEvent: select_level, number: i});
}
